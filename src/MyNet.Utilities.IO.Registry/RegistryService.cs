...
namespace MyNet.Utilities.IO.Registry
{
    public class RegistryService : IRegistryService
    {
        ...
        [SupportedOSPlatform("windows")]
        public IEnumerable<Registry<T>> GetAll<T>(string parentKey) where T : new()
        {
        ...
        [SupportedOSPlatform("windows")]
        public Registry<T>? Get<T>(string parentKey, string key) where T : new() => Get<T>($"{parentKey}\\{key}");

        [SupportedOSPlatform("windows")]
        public Registry<T>? Get<T>(string path) where T : new()
        {
            var type = typeof(T);
            var properties = type.GetPublicProperties().Where(x => x.CanWrite);

            var result = new Registry<T>(path, new T());
        ...
        [SupportedOSPlatform("windows")]
        public void Set<T>(string parentKey, string valueKey, T value) where T : notnull
        {
        ...
        [SupportedOSPlatform("windows")]
        public void AddOrUpdate<T>(Registry<T> value)
        {
            var type = typeof(T);
            var properties = type.GetPublicProperties().Where(x => x.CanWrite);
        ...
        private Func<object, object>? GetConverters(Type type) => !_converters.TryGetValue(type, out var value) ? null : value;
    }
}
