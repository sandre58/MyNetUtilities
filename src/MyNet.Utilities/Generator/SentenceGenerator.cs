...
namespace MyNet.Utilities.Generator
{
    public static class SentenceGenerator
    {
        ...
        private static IEnumerable<string> RandomWordsInString(string words) => words.Split(' ').OrderBy(x => Seed.Next());

        private static IEnumerable<string> RandomWordsInLoremIpsum(bool includePuncation) => includePuncation ? RandomWordsInString(LoremIpsum) : RandomWordsInString(LoremIpsum.Replace(",", ""));

        public static string Words(int wordCount, bool includePunctuation = false) => Words(wordCount, wordCount, includePunctuation);

        public static string Words(int wordCountMin, int wordCountMax, bool includePunctuation = false) => string.Join(" ", RandomWordsInLoremIpsum(includePunctuation).Take(Seed.Next(wordCountMin, wordCountMax)));

        public static string Sentence(int wordCount) => Sentence(wordCount, wordCount);

        public static string Sentence(int wordCountMin, int wordCountMax) => string.Format(CultureInfo.CurrentCulture, "{0}.", Words(wordCountMin, wordCountMax, true)).Replace(",.", ".").Replace("..", "").ToSentence();
        ...
        {
            var source = string.Join(" ", Enumerable.Range(0, Seed.Next(sentenceCountMin, sentenceCountMax)).Select(x => Sentence(wordCountMin, wordCountMax)));
        ...
        public static IEnumerable<string> Paragraphs(int wordCountMin, int wordCountMax, int sentenceCountMin, int sentenceCountMax, int paragraphCountMin, int paragraphCountMax) => Enumerable.Range(0, Seed.Next(paragraphCountMin, paragraphCountMax)).Select(p => Paragraph(wordCountMin, wordCountMax, sentenceCountMin, sentenceCountMax)).ToArray();

        public static string ToSentence(this string input) => input.Length >= 1 ? string.Concat(input.Substring(0, 1).ToUpper(CultureInfo.CurrentCulture), input.Substring(1)) : input.ToUpper(CultureInfo.CurrentCulture);
    }
}
